  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      const res = await fetch('http://localhost:5000/api/auth/register', {
        ...
        body: JSON.stringify({ gender, password, isAgeConfirmed })
      });
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-slate-900 overflow-hidden relative">
      ...
      <motion.div 
        ...
        className="z-10 w-full max-w-md p-8 glass-card bg-white/10 dark:bg-slate-900/50 backdrop-blur-xl border border-white/20 rounded-3xl shadow-2xl m-4"
      >
        <ShreyMascot focusedInput={focusedInput} />
        
        <div className="text-center mb-8 pt-2">
          <h1 className="text-3xl font-bold text-white mb-2">Join <span className="gradient-text">Eros & Psyche</span></h1>
          <p className="text-slate-300 text-sm">Anonymous connections await. No real identity needed.</p>
        </div>
        ...
        <form onSubmit={handleRegister} className="space-y-6">
          ...
          <div className="relative">
            <input 
              ...
              className="w-full bg-slate-800/50 border border-slate-700 text-white rounded-2xl px-4 py-4 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all placeholder:text-slate-500"
            />
            <button 
              type="button" 
          ...
          <div className="flex items-center space-x-3 bg-white/5 p-4 rounded-2xl border border-white/10">
            ...
            />
            <label htmlFor="age" className="text-sm text-slate-300 cursor-pointer flex-1">
              I confirm I am 18+ and agree to the anonymous chat rules.
            </label>
          </div>
          
          <div className="bg-yellow-500/10 border border-yellow-500/30 p-3 rounded-xl flex items-start space-x-3">
             <ShieldAlert className="text-yellow-500 mt-1 flex-shrink-0" size={18} />
             <p className="text-xs text-yellow-200/80">Users may not represent their real identity. Stay cautious. Do not share personal information.</p>
          </div>
          ...
          <motion.button 
            ...
            type="submit"
            disabled={loading}
            className="w-full py-4 rounded-2xl bg-gradient-to-r from-indigo-600 to-pink-600 text-white font-bold text-lg shadow-[0_0_20px_rgba(99,102,241,0.4)] hover:shadow-[0_0_30px_rgba(99,102,241,0.6)] transition-all disabled:opacity-50 mt-4"
          >
        ...
        <p className="text-center text-slate-400 mt-6 text-sm">
          Already have an identity? <Link to="/login" className="text-pink-400 hover:text-pink-300 font-medium">Login here</Link>
        </p>
      </motion.div>
    </div>
  );
